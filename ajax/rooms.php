...
if (isset($_GET['fetch_rooms'])) {
  ...
  while ($room_data = mysqli_fetch_assoc($room_res)) {
    ...
    if (!$settings_r['shutdown']) {
      ...
      $book_btn = "<button onclick='checkLoginToBook($login," . htmlspecialchars($room_data['id']) . ")' class='btn btn-sm w-100 text-white custom-bg shadow-none mb-2'>Book Now</button>";
    }
        ...
        <div class='card mb-4 border-0 shadow'>
          <div class='row g-0 p-3 align-items-center'>
            <div class='col-md-5 mb-lg-0 mb-md-0 mb-3'>
              <img src='" . htmlspecialchars($room_thumb) . "' class='img-fluid rounded'>
            </div>
            <div class='col-md-5 px-lg-3 px-md-3 px-0'>
              <h5 class='mb-3'>" . htmlspecialchars($room_data['name']) . "</h5>
              ...
              <div class='guests'>
                ...
                <span class='badge rounded-pill bg-light text-dark text-wrap'>
                  " . htmlspecialchars($room_data['adult']) . " Adults
                </span>
                <span class='badge rounded-pill bg-light text-dark text-wrap'>
                  " . htmlspecialchars($room_data['children']) . " Children
                </span>
              </div>
            </div>
            <div class='col-md-2 mt-lg-0 mt-md-0 mt-4 text-center'>
              <h6 class='mb-4'>" . htmlspecialchars($room_data['price']) . " VND / night</h6>
              $book_btn
              <a href='room_details.php?id=" . htmlspecialchars($room_data['id']) . "' class='btn btn-sm w-100 btn-outline-dark shadow-none'>More Details</a>
            </div>
          </div>
        </div>
      ";
